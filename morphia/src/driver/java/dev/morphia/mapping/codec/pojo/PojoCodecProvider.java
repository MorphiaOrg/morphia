...
public final class PojoCodecProvider implements CodecProvider {
    static final Logger LOGGER = Loggers.getLogger("codecs.org.bson.codecs.pojo");
    ...
    private <T> MorphiaCodec<T> getPojoCodec(final Class<T> clazz, final CodecRegistry registry) {
        ClassModel<T> classModel = (ClassModel<T>) classModels.get(clazz);
        if (classModel != null) {
            return new BaseMorphiaCodecImpl<T>(classModel, registry, propertyCodecProviders, discriminatorLookup);
            ...
            try {
                ...
                if (clazz.isInterface() || !classModel.getPropertyModels().isEmpty()) {
                    ...
                    return new AutomaticPojoCodec<T>(new BaseMorphiaCodecImpl<T>(classModel, registry, propertyCodecProviders,
                            discriminatorLookup));
                }
            } catch (Exception e) {
                LOGGER.warn(format("Cannot use '%s' with the MorphiaCodec.", clazz.getSimpleName()), e);
                return null;
            }
        }
        return null;
