...
public interface AggregationPipeline {
     ...
     * @return an iterator of the computed results
     */
    <U> Iterator<U> aggregate(String collectionName, Class<U> target, AggregationOptions options,
                              ReadPreference readPreference);
     ...
     * each input document, outputs either one document (for the first n documents) or zero documents
     ...
     * documents from the "joined" collection for processing. The $lookup stage does an equality match
     * between a field from the input documents with a field from the documents of the “joined”
     * collection.  To each input document, the $lookup stage adds a new array field whose elements
     * are the matching documents from the “joined” collection. The $lookup stage passes these
     ...
     * Each output document replaces the array with an element value. For each input document, outputs
     * n documents where n is the number of array elements and can be zero for an empty array.
     *
