...
public final class ReflectionUtils {
     ...
     *
     * @param packageName       the package to scan
     * @param ignoreSubPackages whether to ignore the sub-packages while scanning
     * @return the list of classes
     * @throws IOException            thrown if an error is encountered scanning packages
     * @throws ClassNotFoundException thrown if a class can not be found
     */
    public static Set<Class<?>> getClasses(final String packageName, final boolean ignoreSubPackages) throws IOException,
            ClassNotFoundException {
        final ClassLoader loader = Thread.currentThread()
                                         .getContextClassLoader();
        return getClasses(loader, packageName, ignoreSubPackages);
    }
     ...
     * @throws ClassNotFoundException thrown if a class can not be found
     */
    public static Set<Class<?>> getClasses(final ClassLoader loader, final String packageName, final boolean ignoreSubPackages) throws
            ...
            while (resources.hasMoreElements()) {
                ...
                if (filePath != null) {
                    if ((filePath.indexOf("!") > 0) && (filePath.indexOf(".jar") > 0)) {
                        ...
                        classes.addAll(getFromJARFile(loader, jarPath, path, ignoreSubPackages));
                    } else {
                        classes.addAll(getFromDirectory(loader, new File(filePath), packageName, ignoreSubPackages));
                    }
                }
            }
        }
    ...
    public static Set<Class<?>> getFromJARFile(final ClassLoader loader, final String jar, final String packageName)
        ...
        return getFromJARFile(loader, jar, packageName, true);
    }
    ...
    public static Set<Class<?>> getFromJARFile(final ClassLoader loader, final String jar, final String packageName, final boolean
        ignoreSubPackages) throws IOException, ClassNotFoundException {
        final Set<Class<?>> classes = new HashSet<Class<?>>();
            ...
            do {
                ...
                if (jarEntry != null) {
                    ...
                    if (className.endsWith(".class")) {
                        ...
                        if (classPackageName.equals(packageName) || (!ignoreSubPackages && isSubPackage(classPackageName, packageName))) {
    ...
    public static Set<Class<?>> getFromDirectory(final ClassLoader loader, final File directory, final String packageName)
        ...
        return getFromDirectory(loader, directory, packageName, true);
    }
     ...
     * @throws ClassNotFoundException thrown if a class can not be found
     */
    public static Set<Class<?>> getFromDirectory(final ClassLoader loader, final File directory, final String packageName, final boolean
        ignoreSubPackages) throws ClassNotFoundException {
        final Set<Class<?>> classes = new HashSet<Class<?>>();
        if (directory.exists()) {
            for (final String file : getFileNames(directory, packageName, ignoreSubPackages)) {
    ...
    private static Set<String> getFileNames(final File directory, final String packageName, final boolean ignoreSubPackages) {
        Set<String> fileNames = new HashSet<String>();
        for (File file: directory.listFiles()) {
            if (file.isFile()) {
                fileNames.add(packageName + '.' + file.getName());
            } else if (!ignoreSubPackages){
