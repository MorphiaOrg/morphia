...
public class QueryImpl<T> implements Query<T>
{
    ...
    public Query<T> filter(String condition, Object value)
    {
        ...
        // TODO differentiate between the key/value for maps; we will just get
        // the mf for the field, not which part we are looking for
        ...
        // convert the value to Key (DBRef) if it is a entity/@Reference of the
        // field type is Key
    ...
    private MappedField validate(String prop, Object value)
    {
        ...
        for (int i = 0;;)
        {
            ...
            if (mf == null)
            {
                ...
                if (mf != null)
                {
                    throw new MappingException("The field '" + part + "' is named '" + mf.getName() + "' in '"
                            + this.clazz.getName() + "' " + "(while validating - '" + prop + "'); Please use '"
                            + mf.getName() + "' in your query.");
                }
                else
                {
                    throw new MappingException("The field '" + part + "' could not be found in '"
                            + this.clazz.getName() + "' while validating - " + prop);
                }
            }
            i++;
            if (mf.isMap())
        ...
        if (mf.isSingleValue() && (value != null) && !value.getClass().isAssignableFrom(mf.getType()) &&
                ...
                !value.getClass().getSimpleName().toLowerCase().equals(mf.getType().getSimpleName().toLowerCase()))
        {
                    ...
                    && !(mf.getType().equals(String.class) && value.getClass().equals(ObjectId.class)))
            {
                ...
                log.warning("Datatypes for the query may be inconsistent; searching with an instance of "
                        + value.getClass().getName() + " when the field " + mf.getDeclaringClass().getName() + "."
                        + mf.getClassFieldName() + " is a " + mf.getType().getName());
    ...
    public static class QueryFieldEndImpl<T> implements QueryFieldEnd<T>
    {
        ...
        public QueryFieldEndImpl(String fe, QueryImpl<T> q)
        {
            this.fieldExpr = fe;
            this.query = q;
        }
