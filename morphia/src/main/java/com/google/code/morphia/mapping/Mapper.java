...
public class Mapper {
  ...
  public static MappedField validate(final Class clazz, final Mapper mapper, final StringBuffer origProp, final FilterOperator op,
    ...
    if (validateNames) {
      ...
      for (int i = 0; ; ) {
        ...
        if (i < parts.length && !canQueryPast(mf)) {
          throw new ValidationException(
            "Can not use dot-notation past '" + part + "' could not be found in '" + clazz.getName() + "' while validating - " + prop);
        }
      ...
      if (validateTypes) {
        if ((mf.isSingleValue() && !isCompatibleForOperator(mf.getType(), op, val)) || ((mf.isMultipleValues() && !(isCompatibleForOperator(
          mf.getSubClass(), op, val) || isCompatibleForOperator(mf.getType(), op, val))))) {


          if (log.isWarningEnabled()) {
            final Throwable t = new Throwable();
            final StackTraceElement ste = getFirstClientLine(t);
            log.warning("The type(s) for the query/update may be inconsistent; using an instance of type '" + val.getClass().getName()
              + "' for the field '" + mf.getDeclaringClass().getName() + "." + mf.getJavaFieldName() + "' which is declared as '" + mf
              .getType().getName() + (ste == null ? "'" : "'\r\n --@--" + ste));
  ...
  public static boolean isCompatibleForOperator(final Class<?> type, final FilterOperator op, final Object value) {
    if (value == null || type == null) {
      ...
      return true;
    } else if (op.equals(FilterOperator.IN) && (value.getClass().isArray() || Iterable.class.isAssignableFrom(value.getClass()) || Map.class
