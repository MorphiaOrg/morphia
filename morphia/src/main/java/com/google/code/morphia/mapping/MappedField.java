...
public class MappedField
{
    ...
    private Class subType = null; // the type (T) for the
                                  // Collection<T>/T[]/Map<?,T>
    private Class keyType = null; // the type (T) for the Map<T,?>
    private boolean isSingleValue = true; // indicates the field is a single
                                          // value
    private boolean isMongoType = false; // indicated the type is a mongo
                                         // compatible type (our version of
                                         // value-type)
    private boolean isMap = false; // indicated if it implements Map interface
    private boolean isSet = false; // indicated if the collection is a set (or
                                   // list)
    ...
    {
        for (Class<? extends Annotation> clazz : MappedField.interestingAnnotations)
        {
            addAnnotation(clazz);
        }
        ...
        for (Annotation an : mappingAnnotations.values())
        {
            ...
            {
                MappedField.log.log(Level.WARNING, "There should not be an argument", e);
            }
            catch (Exception e)
            {
                MappedField.log.log(Level.WARNING, "", e);
            }
        }
        ...
        if (type.isArray() || ReflectionUtils.implementsAnyInterface(field.getType(), Iterable.class, Map.class))
        {
            ...
            subType = (type.isArray()) ? type.getComponentType() : ReflectionUtils.getParameterizedClass(field,
                    (isMap) ? 1 : 0);
            if (isMap)
            {
    ...
    public List<String> getLoadNames()
    {
        ...
        AlsoLoad al = (AlsoLoad) mappingAnnotations.get(AlsoLoad.class);
    ...
    /**
     * Indicates whether the annotation is present in the mapping (does not
     * check the java field annotations, just the ones discovered)
     */
    ...
    public void addAnnotation(Class<? extends Annotation> clazz)
    {
        ...
        {
            mappingAnnotations.put(clazz, field.getAnnotation(clazz));
        }
    }
        ...
        if (hasAnnotation(Property.class))
        {
            ...
            if (hasAnnotation(Reference.class))
            {
                ...
                if (hasAnnotation(Embedded.class))
                {
                    ...
                    if (hasAnnotation(Serialized.class))
                    {
    ...
    {
        return name + "; " + mappingAnnotations.toString();
    }
    ...
    /**
     * If the java field is a list/array/map then the sub-type T is returned
     * (ex. List<T>, T[], Map<?,T>
     */
