...
public class BasicDAO<T, K> implements DAO<T, K> {
    ...
    /* (non-Javadoc)
     * @see xyz.morphia.DAO#findOne(xyz.morphia.query.Query)
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see xyz.morphia.DAO#getDatastore()
     */
    @Override
     ...
     */
    @Deprecated
    public xyz.morphia.DatastoreImpl getDs() {
        return ds;
    }
    ...
    protected void initDS(final MongoClient mongoClient, final Morphia mor, final String db) {
        ds = (xyz.morphia.DatastoreImpl) mor.createDatastore(mongoClient, db);
    }
