...
public class ManualMorphiaConfig implements MorphiaConfig {
    ...
    public Boolean autoImportModels() {
        return orDefault(autoImportModels, FALSE);
    }
    ...
    public String toString() {
        return String.format(
                "MorphiaConfig{applyCaps=%s, applyDocumentValidations=%s, applyIndexes=%s, database='%s', codecProvider=%s, " +
                        "collectionNaming=%s, dateStorage=%s, discriminator=%s, discriminatorKey='%s', enablePolymorphicQueries=%s, " +
                        "ignoreFinals=%s, packages=%s, propertyDiscovery=%s, propertyNaming=%s, queryFactory=%s, storeEmpties=%s, " +
                        "storeNulls=%s, uuidRepresentation=%s}",
                applyCaps(), applyDocumentValidations(), applyIndexes(), database(), codecProvider(), collectionNaming(), dateStorage(),
                discriminator(), discriminatorKey(), enablePolymorphicQueries(), ignoreFinals(), packages(), propertyDiscovery(),
                propertyNaming(), queryFactory(), storeEmpties(), storeNulls(), uuidRepresentation());
    }
