...
public class AggregationTest extends TestBase {
    ...
    public void testCase(double serverVersion, String resourceName, String collection, Function<String, Aggregation> pipeline) {
        checkMinServerVersion(serverVersion);
        loadData(collection, resourceName);

        List<Document> actual = removeIds(runPipeline(resourceName, pipeline.apply(collection)));

        assertEquals(actual, loadExpected(resourceName));
