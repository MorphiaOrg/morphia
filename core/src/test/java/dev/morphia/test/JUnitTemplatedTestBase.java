...
/**
 * JUnit 5 version of TemplatedTestBase for template-driven testing
 */
public abstract class JUnitTemplatedTestBase extends JUnitMorphiaTestBase {
     ...
     * Due to JUnit 5 @AfterAll requiring static methods, subclasses should override this
     * as a static method and call checkTestCoverage(SubclassName.class).
     */
    protected void testCoverage() {
        checkTestCoverage(getClass());
    }

    /**
     * Performs the actual test coverage check for the given test class type.
     * This can be called from static @AfterAll methods in subclasses.
     */
    protected static void checkTestCoverage(Class<?> type) {
        ...
        List<File> list = Arrays.stream(Objects.requireNonNull(resourceFolder.list()))
    ...
    protected List<Document> runQuery(ActionTestOptions testOptions, String pipelineTemplate, Query<Document> query) {
        ...
        if (!testOptions.skipActionCheck()) {
            ...
            assertEquals(toJson(target), toJson(document), "Should generate the same query document");
        }
    ...
    private void checkAction(ActionTestOptions testOptions,
        ...
        if (!testOptions.skipActionCheck()) {
            ...
            assertEquals(toJson(action.get(0)), toJson(document), "Should generate the same query document");
            Operations operations = new Operations(getDs(), null, asList(operators), false);
            Document updates = operations.toDocument(getDs());
            assertEquals(toJson(action.get(1)), toJson(updates), "Should generate the same update document");
        }
    }
    ...
    protected void validateTestName(String resourceName) {
        ...
        if (method.isAnnotationPresent(DisplayName.class)) {
            actualName = method.getAnnotation(DisplayName.class).value();
        } else {
            actualName = method.getName();
        }
        String expectedName = loadTestName(resourceName);
        assertEquals(expectedName, actualName,
                "%s#%s does not have the expected name.".formatted(method.getDeclaringClass().getName(),
                        method.getName()));
    }
