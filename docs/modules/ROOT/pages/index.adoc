...
----
dependencies {
    implementation 'dev.morphia.morphia:morphia-core:{version}'
}
----

If you only need Morphia mapping annotations in a shared model jar, you can depend on the annotations-only artifact:

[source,xml,subs="verbatim,attributes"]
----
<dependency>
    <groupId>dev.morphia.morphia</groupId>
    <artifactId>morphia-annotations</artifactId>
    <version>{version}</version>
</dependency>
----
...
----
dependencies {
    implementation 'dev.morphia.morphia:morphia-annotations:{version}'
}
----
