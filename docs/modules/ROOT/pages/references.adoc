...
== Using MorphiaReference

NOTE: `MorphiaReference` is an experimental wrapper type. Its API may change in future releases.

The `MorphiaReference<T>` wrapper type provides an alternative to the `@Reference` annotation.
Instead of annotating a field whose type is the entity itself, you wrap the entity type in `MorphiaReference`:
...
public class Employee {
    ...
    private MorphiaReference<Employee> manager;
    private MorphiaReference<List<Employee>> directReports;
}
----

The reference is resolved lazily by default — calling `.get()` on the wrapper triggers a load from the database.
If the referenced document does not exist, `.get()` returns an empty `Optional` or empty collection depending on the field type.

The wrapper supports the same `idOnly` semantics as the annotation: when only the IDs are relevant and you do not
need to load the referenced entities, set `idOnly = true` on the enclosing entity's `@Reference` annotation.
To suppress exceptions for documents that may not exist, annotate with `ignoreMissing = true`.

For full API options, see the
link:javadoc/dev/morphia/mapping/experimental/MorphiaReference.html[MorphiaReference javadoc].