...
Starting 2.3, you can provide your own `CodecProvider` to customize how Morphia handles the types you're interested.
...
You can register your custom `CodecProvider` implementation via the `MapperOptions.Builder#codecProvider()`
method.
For details on how to write a `Codec` and a `CodecProvider`, please see {docsRef}/drivers/java/sync/current/fundamentals/data-formats/codecs/[these docs].
