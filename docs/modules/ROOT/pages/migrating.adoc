...
=== Configuration
Morphia 2.4 sees the introduction of file-based configuration instead of the programmatic configuration currently in use.  The file
...
xref:configuration.adoc[configuration section].  In order to aid in your migration to the new configuration file, a couple of options
are available to you:

1.  Upon initializing Morphia using the traditional method of passing in `MapperOptions` to `#createDatastore()`, Morphia will log a
warning about the deprecation of that approach and will output your current configuration in config file format to the logs which you can
simply copy and paste in to the configuration file and update to use the
link:++javadoc/dev/morphia/Morphia.html#createDatastore(com.mongodb.client.MongoClient)++[new method].
2.  You can manually generate that content by calling `MapperOptions#toConfigFormat(String database, boolean showComplete)` on your
instance.  This will return a `String` which can be copied in to your configuration file.
