...
The new API will attempt to redirect most calls to use the new link:javadoc/dev/morphia/query/experimental/filters/Filters.html[Filters] based approach.
...
method instead of `field()` but it is used in much the same fashion.  `and()` and `or()` take a https://docs.oracle
.com/javase/8/docs/technotes/guides/language/varargs.html[`varargs`] parameter of type `Criteria` so you can include as many filters as necessary.
