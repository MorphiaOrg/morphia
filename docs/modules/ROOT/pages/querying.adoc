...
To save such instances back consider using link:++javadoc/dev/morphia/Datastore#merge-T-.html++[`Datastore#merge(T)`]
====
...
Ordering the results of a query is done via link:++javadoc/dev/morphia/query/Query.html#order-java.lang.String-++[`Query.order(String)`].
