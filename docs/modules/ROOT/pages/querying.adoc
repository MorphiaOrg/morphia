...
To save such instances back consider using xref:javadoc:dev/morphia/Datastore#merge-T-.html#[`Datastore#merge(T)`]
====
...
Ordering the results of a query is done via xref:javadoc:dev/morphia/query/Query.html#order-java.lang.String-#[`Query.order(String)`].
...
Using your regular `Datastore` reference, you can cast to `AdvancedDatastore` and pass an existing query document to be used instead of building it via the API.
